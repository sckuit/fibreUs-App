...
## Recent Changes
- **2025-11-07**: Implemented dynamic sitemap.xml endpoint for SEO optimization. GET /sitemap.xml automatically lists all public URLs (quotes, invoices, projects, tickets with shareTokens) in Google's sitemap XML format with lastmod timestamps, priorities, and change frequencies. Updated robots.txt to reference sitemap dynamically using request host. Sitemap includes homepage (priority 1.0), quotes/invoices (priority 0.8, monthly), projects (priority 0.7, weekly), and tickets (priority 0.6, weekly).
