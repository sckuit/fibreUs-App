...
## Recent Changes
- **2025-11-14**: Added print functionality to quote, invoice, and promotional quote edit pages with PrintPreviewDialog component. Features include: (1) Print button in QuoteBuilder, InvoiceBuilder, and PromoQuoteBuilder toolbars next to Download PDF, (2) Print preview dialog with configurable options (orientation: portrait/landscape, margins: 0.5in/1in/1.5in), (3) Browser-native window.print() integration with dynamic @page CSS rules, (4) Print-optimized CSS using @media print that preserves Radix UI portals and hides dialog chrome, (5) Reuses existing QuotePreview/InvoicePreview components for consistency. Print flow: user clicks Print → adjusts settings → prints via browser dialog with configured page setup.
...
- **Quote/Invoice Printing**: Browser-native print functionality with configurable orientation and margins via PrintPreviewDialog component. Supports portrait/landscape orientation and three margin options (0.5in, 1in, 1.5in) with dynamic @page CSS rules.
